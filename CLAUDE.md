...
- **Consigne du client : déployer SYSTÉMATIQUEMENT en production, jamais en preview.** Mécanisme : `vercel.json` → `ignoreCommand` ne laisse builder QUE `claude/setup-swiss-serenity-plus-Fm7s6` (toute autre branche = build skippé). Donc après chaque lot : `git push origin HEAD:claude/setup-swiss-serenity-plus-Fm7s6` (déploie en `target: production`) ; on peut aussi pousser la branche de session (elle ne déclenche aucun déploiement). Toujours `git fetch` + vérifier le fast-forward avant de pousser. Projet Vercel : `mireille` (`prj_PfoVB46yczr5aaSteAdlRip99BCT`, team `team_IiMX8Y4Z9VI3RQiEqmFEgbd0`) ; vérifiable via le MCP Vercel (`list_deployments` → le déploiement Fm7s6 a `target: production`, les autres ne sont plus créés).
