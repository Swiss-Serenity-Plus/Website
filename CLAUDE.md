...
- **Branche** : `claude/setup-swiss-serenity-plus-Fm7s6` est **la branche par défaut du dépôt GitHub** ET la branche de production connectée à Vercel (`target: production`). **On pousse UNIQUEMENT dessus.** (En session web, la branche locale s'appelle `claude/<random>` ; le pattern : committer en local, `git branch -f claude/setup-swiss-serenity-plus-Fm7s6 HEAD`, puis `git push origin claude/setup-swiss-serenity-plus-Fm7s6`.)
- ⛔ **NE PAS pousser la branche de session `claude/<random>`** : sur Vercel elle déclenche un déploiement **preview** (`target: null`) en doublon de chaque commit, ce qui fait croire que le travail est « en preview ». Pour calmer le stop-hook sans pousser de preview : `git branch --set-upstream-to=origin/claude/setup-swiss-serenity-plus-Fm7s6` une fois (l'upstream local pointe alors sur la prod, plus d'alerte « unpushed »).
