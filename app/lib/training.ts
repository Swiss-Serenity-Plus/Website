// Lecture de la base de formations Notion (Notion_Training_Database_ID).
//
// Mutualise la logique entre l'API /api/training-posts (console admin) et les
// routes dédiées /formation/[id]. Conversion complète des blocs Notion en HTML :
// paragraphes, titres, listes, citations, code (copiable), tableaux (avec fetch
// des lignes enfants), callouts, toggles, to-do, images, embed/video/bookmark
// Tella (iframes 16:9).
import "server-only";
...
type NotionProp = { type?: string; url?: string | null; rich_text?: RT[]; title?: RT[] };

function findTellaUrl(properties: Record<string, NotionProp>): string {
...
function findTitle(properties: Record<string, NotionProp>): string {
  for (const prop of Object.values(properties)) {
    if (prop.type === "title" && prop.title) return readRT(prop.title);
  }
  return "";
}
...
// Renvoie l'ID de la base de formations, ou null si la configuration manque.
export function trainingDatabaseId(): string | null {
  if (!process.env.NOTION_TOKEN) return null;
  return process.env.Notion_Training_Database_ID ?? null;
}

// ── API publique du module ───────────────────────────────────────────────────

// Liste des items de la base (triés par date de création ascendante).
export async function getTrainingItems(): Promise<TrainingItem[]> {
  const dbId = trainingDatabaseId();
  if (!dbId) return [];
...
// Corps HTML d'un item (blocs de la page convertis).
export async function getTrainingBodyHtml(pageId: string, title = ""): Promise<string> {
